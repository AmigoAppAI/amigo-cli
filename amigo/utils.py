...
amigo_dir_path = Path.home() / ".amigo"

# package_name should always be "amigo" - but this will work if package name is changed
...
def check_version():
    ...
    try:
        response = requests.get("https://pypi.org/pypi/amigo/json")
        data = response.json()
        latest_version = data["info"]["version"]
        current_version = pkg_resources.require("amigo")[0].version

        if packaging.version.parse(current_version) < packaging.version.parse(
            latest_version
        ):
            ctx.stream.send(
                f"Version v{latest_version} of Amigo is available. If pip was used to"
                " install Amigo, upgrade with:",
                style="error",
            )
            ctx.stream.send("pip install --upgrade amigo", style="warning")
