...
from amigo.code_feature import CodeFeature, count_feature_tokens
from amigo.errors import AmigoError
from amigo.llm_api_handler import model_context_size, model_price_per_1000_tokens
...
client = chromadb.PersistentClient(path=str(amigo_dir_path / "chroma"))


class AmigoEmbeddingFunction(EmbeddingFunction[Embeddable]):
    def __call__(self, input: Embeddable) -> Embeddings:
        if not all(isinstance(item, str) for item in input):
            raise AmigoError("AmigoEmbeddings only enabled for text files")
...
class Collection:
    def __init__(self, embedding_model: str):
        self._collection = client.get_or_create_collection(
            name=f"amigo-{embedding_model}",
            embedding_function=AmigoEmbeddingFunction(),
        )
...
async def get_feature_similarity_scores(
    ...
    if max_model_tokens is None:
        raise AmigoError(f"Missing model context size for {embedding_model}.")
