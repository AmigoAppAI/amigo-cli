...
def get_amigo_comment(filename: Path) -> Optional[Tuple[int, str]]:
    with open(filename, "r") as file:
        for line_number, line in enumerate(file, 1):
            if line.startswith("# @amigo"):
                comment_text = line.split("@amigo", 1)[1].strip()
...
def modify_file_lines(*, path: Path, user_prompt: str, code_lines: List[str]):
    ...
    for _line_number, line in enumerate(file_lines, 1):
        if line.strip() == f"# @amigo {user_prompt}":
    ...
    file_lines.insert(line_number - 1, f"# [completed] @amigo {user_prompt}\n")
...
def process_file_change(path: Path):
    # Get @amigo comment
    comment = get_amigo_comment(path)
    if comment is None:
        return
