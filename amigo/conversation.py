...
from amigo.llm_api_handler import (
    TOKEN_COUNT_WARNING,
    count_tokens,
    get_max_tokens,
    prompt_tokens,
)
from amigo.parsers.file_edit import FileEdit
from amigo.parsers.parser import ParsedLLMResponse
from amigo.session_context import SESSION_CONTEXT
from amigo.transcripts import ModelMessage, TranscriptMessage, UserMessage
from amigo.utils import add_newline


class AmigoAssistantMessageParam(ChatCompletionAssistantMessageParam):
...
class Conversation:
    ...
    async def display_token_count(self):
        ...
        if "gpt-4" not in config.model:
            stream.send(
                "Warning: Amigo has only been tested on GPT-4. You may experience"
            ...
            if "gpt-3.5" not in config.model:
                stream.send(
                    "Warning: Amigo does not know how to calculate costs or context"
                    " size for this model.",
                    style="warning",
                )
